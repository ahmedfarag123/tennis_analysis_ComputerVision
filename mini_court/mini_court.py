...
class MiniCourt():
    ...
    def draw_background_rectangle(self,frame):
        ...
        out[mask] = cv2.addWeighted(frame, alpha, shapes, 1 - alpha, 0)[mask] #out now contains the original image with a semi-transparent white rectangle drawn on it, only in the area defined by the mask. Without the [mask] at the end, the blending would be applied to the entire image, which could affect parts of the image where you don't want the rectangle to be drawn. By using [mask], you're restricting the operation to only the area inside the rectangle.
